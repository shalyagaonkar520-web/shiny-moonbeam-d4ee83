...
export default function HotelCoastalCrownMenuPage() {
  useSEO(
    'Coastal Crown Hotel Menu - Authentic Coastal & Tandoor Cuisine',
    'Browse Coastal Crown hotel menu: Kundapur Chicken, Neer Dosa, Kori Roti, Chicken & Mutton Main Course, Veg Dishes, Egg Curries and Biryani specials.'
  );

  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState('');
  const [dietFilter, setDietFilter] = useState<DietFilter>('all');
  const [activeCategory, setActiveCategory] = useState(HOTEL_COASTAL_CROWN_MENU[0].title);
  ...
  const handleAddToCart = (
    item: CoastalCrownMenuItem,
    categoryTitle: string
  ) => {
    playSound(SOUNDS.ADD_TO_CART);
    const itemId = `coastal-${item.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
    addItem({
      id: itemId,
  ...
  const filteredMenu = useMemo(() => {
    ...
    return HOTEL_COASTAL_CROWN_MENU.map((category) => {
  ...
  const totalItemsCount = useMemo(
    () => HOTEL_COASTAL_CROWN_MENU.reduce((sum, c) => sum + c.items.length, 0),
    []
  );
  ...
  return (
    <div className="min-h-screen bg-[#faf7f5] pb-32">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-rose-100 shadow-xs">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <button
              onClick={() => navigate('/')}
              className="p-2 rounded-full hover:bg-rose-50 active:scale-95 transition-all text-gray-800"
              aria-label="Back to home"
            >
              <ChevronLeft className="w-6 h-6" />
            </button>
            <div className="truncate">
              <h1 className="text-base font-black text-gray-900 truncate tracking-tight">Coastal Crown</h1>
              <div className="flex items-center gap-1.5 text-[11px] font-semibold text-rose-600">
                <MapPin className="w-3 h-3 text-rose-500" />
                <span>Yellapur • Coastal & Tandoor</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="relative p-2.5 rounded-full bg-rose-50 text-rose-600 hover:bg-rose-100 active:scale-95 transition-all"
              aria-label="View Cart"
            >
              <ShoppingBag className="w-5 h-5" />
              {cartItemCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-[#e11d48] text-white text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center shadow-md animate-scale">
                  {cartItemCount}
                </span>
              )}
            </button>
        ...
        <div className="relative rounded-3xl overflow-hidden shadow-lg border border-rose-100 bg-white">
          <div className="relative h-44 sm:h-56 w-full overflow-hidden">
            ...
            <div className="absolute bottom-3 left-4 right-4 text-white">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-rose-600/90 text-[11px] font-bold tracking-wide uppercase shadow-xs mb-1">
                <Sparkles className="w-3 h-3" /> Official Menu
              </div>
              <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight drop-shadow-md">
                Coastal Crown Hotel
              </h2>
              <p className="text-xs sm:text-sm text-gray-200 font-medium">
                Authentic Kundapur specials, rich curries & aromatic biryanis
              </p>
            </div>
          </div>

          <div className="p-3.5 bg-gradient-to-r from-rose-50/70 via-white to-rose-50/70 flex items-center justify-between text-xs font-semibold text-gray-700">
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1 text-emerald-700 font-bold">
                <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" /> 4.6 (280+ ratings)
              </span>
              <span className="text-gray-300">•</span>
              <span className="text-gray-600">Top 5 Selection / Main Course</span>
            </div>
            <span className="font-bold text-rose-600">{totalItemsCount} dishes</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 mt-4 space-y-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search Coastal Crown dishes (e.g. Kundapur, Neer Dosa, Sukka...)"
            className="w-full pl-10 pr-9 py-2.5 bg-white border border-rose-100 rounded-2xl text-xs sm:text-sm font-medium focus:outline-none focus:ring-2 focus:ring-rose-400/50 shadow-xs"
          />
          {searchQuery && (
            <button
      ...
      <div className="sticky top-[57px] z-30 bg-[#faf7f5]/95 backdrop-blur-md py-2 border-b border-rose-100/60 mt-3">
        <div className="max-w-4xl mx-auto px-4 flex items-center gap-2 overflow-x-auto no-scrollbar">
          {HOTEL_COASTAL_CROWN_MENU.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-2xl text-xs font-bold shrink-0 transition-all flex items-center gap-1.5 ${
                  isActive
                    ? 'bg-[#e11d48] text-white shadow-sm scale-102'
                    : 'bg-white text-gray-700 border border-rose-100/80 hover:bg-rose-50'
                }`}
              >
                <span>{icon}</span>
                <span>{cat.title}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  {category.items.map((item) => {
                    ...
                    return (
                      <div
                        key={item.name}
                        className="bg-white rounded-2xl p-3.5 border border-rose-100 shadow-xs hover:shadow-md transition-all flex gap-3.5 relative overflow-hidden"
                      >
                        {/* Dish Details */}
                          ...
                          <div className="absolute bottom-1.5 left-2 right-2">
                            {qty === 0 ? (
                              <button
                                ...
                                className="w-full py-1.5 bg-white/95 backdrop-blur-xs text-[#e11d48] border border-[#e11d48]/40 rounded-xl text-xs font-black uppercase tracking-wider shadow-md hover:bg-[#e11d48] hover:text-white transition-all active:scale-95 flex items-center justify-center gap-1"
                              >
                                <Plus className="w-3.5 h-3.5" /> ADD
                              </button>
                            ) : (
                              <div className="w-full py-1 bg-[#e11d48] text-white rounded-xl text-xs font-black shadow-md flex items-center justify-between px-2">
                                <button
                                  onClick={() => {
      ...
      <AnimatePresence>
        {cartItemCount > 0 && (
          ...
          >
            <button
              onClick={() => navigate('/checkout')}
              className="w-full p-3.5 rounded-2xl bg-gradient-to-r from-rose-600 to-pink-600 text-white shadow-xl flex items-center justify-between active:scale-98 transition-transform"
            >
              <div className="flex items-center gap-2.5">
              ...
              <div className="flex items-center gap-1.5 text-xs font-black uppercase tracking-wider bg-white text-[#e11d48] px-3.5 py-2 rounded-xl shadow-xs">
                <span>Checkout</span>
      ...
      {showScrollTop && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-24 right-4 z-30 p-2.5 rounded-full bg-white/95 text-gray-700 shadow-md border border-rose-100 hover:bg-rose-50 active:scale-90 transition-all"
